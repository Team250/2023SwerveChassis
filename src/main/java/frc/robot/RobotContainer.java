...
import frc.robot.commands.Autos.LLAutos.*;
...
// private final Joystick joystick1 = new Joystick(0);
  ...
  */
  private RobotContainer() {
    ...
    //To deliver then move on trajectory, make two paths, one approaches the grid and the other 
    //moves how you want it to.
    //Make it a sequential command
    //In theory Approach traj should be the same for each traj
    m_chooser.addOption("Loading Mobility Charge", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm), new LoadMobilityCharge(m_driveTrain)));
    m_chooser.addOption("Loading Mobility", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new LoadMobility(m_driveTrain)));
    m_chooser.addOption("Loading Mobility Aquire", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new LoadMobilityAquire(m_driveTrain)));
    m_chooser.addOption("Center Mobility Charge", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new CenterMobilityCharge(m_driveTrain)));
    m_chooser.addOption("Center Mobility", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new CenterMobility(m_driveTrain)));
    m_chooser.addOption("Opp Mobility Charge", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new RightMobilityCharge(m_driveTrain)));
    m_chooser.addOption("Opp Mobility", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new RightMobility(m_driveTrain)));
    m_chooser.addOption("Move Forward", new SequentialCommandGroup(new ArmToHighNode(m_arm), new ApproachGrid(m_driveTrain),new OpenArm(m_arm),new MoveForward(m_driveTrain)));
...
//Auto balance and turn to 90,180,270,0
final POVButton xboxButton180 = new POVButton(driverController, 180);        
xboxButton180.whileTrue(new AutoBalance(m_driveTrain,m_gyro).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
